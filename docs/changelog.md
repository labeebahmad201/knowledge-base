...
- [Language Choice: How to Pick the Right Language for the Job](./software-engineering/language-choice.md) - the runtime model as the sharpest difference (threads+GC, event loop, goroutines, GIL, actors, ownership without GC), workload-to-family mapping with each language's founding problem, innovation tokens and total cost of ownership, the 5-10 year horizon and migration economics, polyglot cost, and a repeatable decision framework with a scorecard and worked examples.
