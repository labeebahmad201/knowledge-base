...
- [Why Go Has No Type Inheritance](./software-engineering/go/why-go-has-no-type-inheritance.md) - the Go FAQ's "Why is there no type inheritance?" answer quoted in full and then explained claim by claim: relationships "could be derived automatically", implicit interface satisfaction with no `implements` keyword, the four advantages (many interfaces at once without the diamond problem, lightweight one- or zero-method interfaces, interfaces added after the fact, no hierarchy to manage), the type-safe Unix pipes payoff built on `io.Writer` (`fmt.Fprintf`, `bufio`, `image`, `gzip`), what replaces inheritance (interfaces for subtyping and dispatch, embedding for reuse, with the key point that embedding is not subtyping), the costs (exact method signatures, no covariant return types, dispatch only through interfaces), and a decision guide mapping `extends` needs to Go tools.
- [The Shift from Class Hierarchy to Composition: Why the Industry Moved](./software-engineering/shift-from-class-hierarchy-to-composition.md) - the short, grounded version of why design moved from inheritance to composition: the four problems with inheritance (fragile base class, rigid is-a modeling, combinatorial explosion, static inflexibility) and the four benefits of composition (has-a modeling, low coupling, runtime adaptability, modularity), each tied to its source (GoF *Design Patterns* p. 20 and p. 175, Bloch *Effective Java* Item 18, Snyder OOPSLA 1986, the Go FAQ, React's "Composition vs Inheritance"), plus the industry confirmation (GoF, React, Go, Rust).
- [Abstract Factory: Creating a Family of Objects Without Naming Them](./software-engineering/abstract-factory.md) - the GoF creational pattern (Intent and Motivation from *Design Patterns*, 1994, also known as Kit): the two separate problems it solves (the client naming a concrete class, fixed by the abstract product; nothing keeping the products matched, fixed by the abstract factory), why the key word is "families", the abstract product as the type the client binds to (and why Java forces the annotation), the abstract factory as the single source that keeps the set consistent, the Motif/Presentation Manager worked example, the book's class diagram, and the asymmetric cost (a new family is cheap, a new product kind is expensive).
...
- [The Software Factory Mirage](./perspectives/the-software-factory-mirage.md) - the "coding is solved" narrative and the "you are holding it wrong" rebuttal describe the same tool, why the software factory is a mirage for most people today, the genuinely mixed evidence, and choosing your own integration level while protecting domain expertise.
